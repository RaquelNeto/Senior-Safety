...
public class SettingsActivity extends Activity {
        ...
        buttonSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (validateData()) {
                    retrieveContactInfo();
                    saveContact();
                    Toast.makeText(SettingsActivity.this, "Contacto Guardado!", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        });
    ...
    private boolean validateData() {
        if (textName.getText().toString().equals("")) {
            Toast.makeText(SettingsActivity.this, "\n" + "Digite um Nome de Contato.", Toast.LENGTH_SHORT).show();
            return false;
        }


        if (!validateCell(textCell.getText().toString())) {
            Toast.makeText(SettingsActivity.this, "Digite um número de telemóvel válido.", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (!validateEmail()) {
            Toast.makeText(SettingsActivity.this, "\n" + "Por favor insira um e-mail válido.", Toast.LENGTH_SHORT).show();
            return false;
        }
    ...
    private boolean validateCell(String c) {
        ...
        String expression = "(9[1236][0-9]) ?([0-9]{3}) ?([0-9]{3})";
